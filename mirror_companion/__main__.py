"""CLI: python3 -m mirror_companion pair --session <id>"""
...
from mirror_companion.server import BridgeState, serve
from mirror_companion.session_log import find_session_dir
from mirror_companion.watch import watch_inbox


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="mirror-companion")
    sub = parser.add_subparsers(dest="cmd", required=True)
    pair = sub.add_parser("pair", help="Start the LAN bridge and print a QR")
    pair.add_argument("--session", required=True, help="Agent session id")
    pair.add_argument("--cwd", default=str(Path.cwd()), help="Workspace for the agent CLI's --resume")
    pair.add_argument("--port", type=int, default=8787)
    pair.add_argument("--agent-home", default=str(Path.home() / ".grok"))
    ...
    watch.add_argument("--session", required=True, help="Agent session id")
    watch.add_argument("--agent-home", default=str(Path.home() / ".grok"))
    ...
    if args.cmd == "watch":
        if not valid_session_id(args.session):
            ...
            return 2
        watch_inbox(Path(args.agent_home), args.session, interval=args.interval)
        return 0
    if args.cmd != "pair":
        return 2
    if not valid_session_id(args.session):
    ...
    session_dir = find_session_dir(agent_home, args.session)
    if session_dir is None:
        print("session not found under ~/.grok/sessions", file=sys.stderr)
        return 1

    tls_dir = agent_home / "mirror" / "tls"
    ...
    out()
    out("Mirror pairing — v3.0 - multiple sessions")
    ...
    if args.quiet:
        ...
        try:
            ...
            sys.stdout.flush()
            png = agent_home / "mirror" / "pair.png"
    ...
    state = BridgeState(
        ...
        require_apple=args.require_apple,
        apple_bind_path=(
            agent_home / "mirror" / f"apple_bind.{args.session}.json" if args.require_apple else None
        ),
    )
